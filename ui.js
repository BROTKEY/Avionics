class Tables {
  constructor() {
    ...
    const bnotwo = document.getElementById("BNOtwo").getContext("2d");
    this.BNOTWO = new Chart(bnotwo, {
      type: "line",
      data: {
        labels: this.labels,
        datasets: [
  ...
  async update() {
    ...
    var BNOtwo = response.data[2];
    console.log(response.data);
    this.BNO.data.labels = BNO.map((x) => x[1]);
    this.BNO.data.datasets[0].data = BNO.map((i) => i[0][2]);
    this.BNO.data.datasets[1].data = BNO.map((i) => i[0][1]);
    this.BNO.data.datasets[2].data = BNO.map((i) => i[0][0]);
    this.BNO.update();
    ...
    this.BMP.update();
    this.BNOTWO.data.labels = BNOtwo.map((x) => x[1]);
    this.BNOTWO.data.datasets[0].data = BNOtwo.map((i) => i[0][0]);
    this.BNOTWO.data.datasets[1].data = BNOtwo.map((i) => i[0][1]);
    this.BNOTWO.data.datasets[2].data = BNOtwo.map((i) => i[0][2]);
    this.BNOTWO.update();
